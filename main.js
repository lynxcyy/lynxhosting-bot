...
require("http").createServer((_, res) => res.end("Uptime!")).listen(8800)
...
prefix = new RegExp('^[' + (opts['prefix'] || '芒鈧絰zXZ/!#$%+脗拢脗垄芒鈥毬偮脗掳=脗露芒藛鈥犆冣€斆兟访忊偓芒藛拧芒艙鈥溍偮┟偮�:;?&.\\-').replace(/[|\\{}()[\]^$+*?.\-\^]/g, '\\$&') + ']')
...
const phoneNumber = await question(chalk.bgWhite(chalk.blue("Start with your country's WhatsApp code, Example : 62xxx\n")))
...
function clearTmp() {
  const tmp = [os.tmpdir(), path.join(__dirname, './.npm')]
  const filename = []
...
setInterval(async () => {
    const q = { "key": { "remoteJid": "status@broadcast", "participant":"628123891243@s.whatsapp.net", "fromMe": false, "id": "" },
	"message": { "conversation": "Sukses mencadangkan database.json ✅" }}
    let sesi = await fs.readFileSync('./database.json')
    return await conn.sendMessage('6283877118785@s.whatsapp.net', { document: sesi, mimetype: 'application/json', fileName: 'database.json' }, { quoted: q })
  }, 60 * 60 * 1000)

const ownerID = '6281222844295@s.whatsapp.net';
const groupIDs = ['120363269606073103@g.us', 
                   ''
                   ]; 
...
async function connectionUpdate(update) {
  ...
  if (connection == 'open') {
    ...
    const welcomeMessage = `Bot berhasil terhubung pada ${new Date().toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}`;
}
