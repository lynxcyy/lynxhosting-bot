let handler = async (m, { conn, usedPrefix }) => {
    ...
    if (user.umpan == 0) {
        const soad = [{
            ...
            buttonParamsJson: JSON.stringify({
                display_text: 'Beli Umpan',
                id: `${usedPrefix}buy `
            })
        }]
        conn.sendAIMessage(m.chat, soad, m, {
            content: 'KAMU TIDAK MEMPUNYAI UMPAN!',
    ...
    if (user.fishingrod == 0) {
        const soad = [{
            name: 'quick_reply',
            buttonParamsJson: JSON.stringify({
                display_text: 'Beli Fishingrod',
                id: `${usedPrefix}buy `
            })
        }]
        conn.sendAIMessage(m.chat, soad, m, {
            content: 'KAMU TIDAK MEMPUNYAI FISHINGROD!',
            footer: global.set.footer,
        })
        return // Menambahkan return untuk menghentikan eksekusi
    }
